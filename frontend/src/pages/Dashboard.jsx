...
export default function Dashboard() {
  ...
  // estados da interacao do aluno
  const [selectedOption, setSelectedOption] = useState(null);
  const [reasoning, setReasoning] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  // estados da submissao gamificada
  const [hasSubmitted, setHasSubmitted] = useState(false);
  const [submissionResult, setSubmissionResult] = useState(null);

  // estados do carrossel/calendario
  ...
  // estados da timeline e leitura
  const [timelineData, setTimelineData] = useState(null);
  const [isReadOnly, setIsReadOnly] = useState(false);

  // mapeia o historico para acesso o(1) pela data
  ...
  // abre e carrega os dados do ranking
  const handleOpenRanking = async () => {
    ...
    try {
      ...
      alert("ERRO_ Nao foi possivel acessar o ranking da turma.");
    } finally {
  ...
  // busca o desafio e a timeline blindando contra memory leaks
  useEffect(() => {
    let isMounted = true;
    ...
    // cleanup function para evitar atualizacao de estado em componente desmontado
    return () => {
      isMounted = false;
    };
  }, [navigate]);

  // processa o envio da resposta gamificada
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(error);
      alert("ERRO_ DE CONEXAO. O servidor recusou a injeção de dados.");
    } finally {
      setIsSubmitting(false);
    }
  };

  // funcoes de arrastar o mouse do calendario
  ...
  const handleDayClick = async (day, dayData, isToday) => {
    ...
    try {
      ...
      console.error(error);
      alert("ERRO_ Nao foi possivel carregar este desafio.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[var(--color-cyber-dark)] py-8 md:py-12 font-sans text-white overflow-x-hidden relative" style={{ backgroundImage: 'radial-gradient(rgba(0, 255, 102, 0.15) 2px, transparent 2px)', backgroundSize: '30px 30px' }}>
      ...
      <header className="relative z-10 max-w-6xl mx-auto px-6 md:px-12 flex flex-col lg:flex-row justify-between items-center mb-12 gap-6">
        ...
        <div className="flex flex-wrap items-center justify-center lg:justify-end gap-3 font-mono font-bold text-xs tracking-widest uppercase">
          ...
          <button onClick={handleOpenRanking} className="flex items-center h-10 border-[3px] border-yellow-400 bg-yellow-400 text-black px-4 hover:bg-transparent hover:text-yellow-400 transition-colors shadow-[4px_4px_0px_0px_rgba(250,204,21,0.5)] whitespace-nowrap">
            [ RANKING ]
          </button>
          ...
          <button onClick={() => { localStorage.removeItem('@DesafioDoDia:token'); navigate('/login'); }} className="flex items-center h-10 border-[3px] border-[var(--color-cyber-magenta)] bg-black text-white hover:bg-[var(--color-cyber-magenta)] transition-colors px-4 whitespace-nowrap cursor-pointer">
            &gt; SAIR
          </button>
        </div>
      </header>
      ...
      <div ref={wrapperRef} className="relative z-10 w-full flex justify-center mb-12 px-8">
        <div ref={sliderRef} onMouseDown={handleMouseDown} onMouseLeave={handleMouseLeave} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove} style={{ width: trackWidth > 0 ? `${trackWidth}px` : '100%', paddingLeft: `${sidePadding}px`, paddingRight: `${sidePadding}px`, maskImage: 'linear-gradient(to right, transparent 0, black 32px, black calc(100% - 32px), transparent 100%)', WebkitMaskImage: 'linear-gradient(to right, transparent 0, black 32px, black calc(100% - 32px), transparent 100%)' }} className={`flex items-start overflow-x-auto pb-6 pt-4 gap-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}>
          {days.map((day) => {
            ...
            return (
              <div key={day} id={isToday ? 'day-today' : `day-${day}`} className="flex flex-col items-center shrink-0 cursor-pointer hover:scale-110 transition-transform" onClick={() => handleDayClick(day, dayData, isToday)}>
      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 md:px-12 mb-20">
        {loading ? (
          <div className="bg-black border-[4px] border-[var(--color-acid-green)] shadow-[8px_8px_0px_0px_var(--color-acid-green)] p-12 text-center">
            <h2 className="font-mono text-[var(--color-acid-green)] text-xl animate-pulse tracking-widest uppercase">&gt; CARREGANDO_SISTEMA...</h2>
          </div>
        ) : !challenge ? (
          <div className="bg-black border-[4px] border-[var(--color-cyber-magenta)] shadow-[8px_8px_0px_0px_var(--color-cyber-magenta)] p-12 text-center">
            <h2 className="font-display text-[var(--color-cyber-magenta)] text-2xl font-black uppercase">ERRO_404</h2>
            <p className="font-mono text-white mt-4 uppercase tracking-widest">Nenhum desafio encontrado para a sua turma hoje.</p>
          </div>
        ) : hasSubmitted && submissionResult ? (
          <div className={`border-[4px] shadow-[8px_8px_0px_0px] p-12 text-center flex flex-col items-center animate-fade-in-up ${submissionResult.is_correct ? 'bg-black border-[var(--color-acid-green)] shadow-[var(--color-acid-green)]' : 'bg-black border-[var(--color-cyber-magenta)] shadow-[var(--color-cyber-magenta)]'}`}>
            <div className={`text-6xl mb-6 ${submissionResult.is_correct ? 'text-[var(--color-acid-green)]' : 'text-[var(--color-cyber-magenta)]'}`}>
              {submissionResult.is_correct ? '🟢' : '🔴'}
            </div>
            <h2 className={`font-display text-3xl md:text-5xl font-black uppercase tracking-tighter ${submissionResult.is_correct ? 'text-[var(--color-acid-green)]' : 'text-[var(--color-cyber-magenta)]'}`}>
          ...
          <div className="bg-black border-[4px] border-[var(--color-acid-green)] shadow-[8px_8px_0px_0px_var(--color-acid-green)] overflow-hidden flex flex-col">
            <div className="bg-[var(--color-acid-green)] border-b-[4px] border-[var(--color-acid-green)] px-4 py-2 flex justify-between items-center">
              ...
              <span className="font-mono font-black text-black text-sm tracking-widest uppercase">CHALLENGE_TODAY.EXE</span>
            </div>
            <div className="p-6 md:p-8 flex justify-between items-start">
              <div className="flex flex-col gap-4">
                <h2 className="font-display text-2xl md:text-3xl font-black uppercase text-white mt-2">{challenge.title}</h2>
              </div>
            </div>
            ...
            <div className="p-6 md:p-8 bg-black">
              <p className="font-mono text-[var(--color-acid-green)] font-bold text-sm tracking-widest mb-6 uppercase">&gt; ESCOLHA A RESPOSTA CORRETA:</p>
              <div className="flex flex-col gap-4 font-sans">
                {challenge.options.map((optionText, index) => {
                  const letra = String.fromCharCode(65 + index);
                  const isSelected = selectedOption === index;
                  return (
                    <button key={index} disabled={isReadOnly} onClick={() => setSelectedOption(index)} className={`flex items-center w-full text-left border-[3px] p-3 md:p-4 transition-all group ${isSelected ? 'bg-[#001100] border-[var(--color-acid-green)] text-white shadow-[4px_4px_0px_0px_var(--color-acid-green)] translate-y-[-2px] translate-x-[-2px]' : isReadOnly ? 'bg-black border-[#333] text-gray-600 cursor-not-allowed' : 'bg-black border-[#333] text-gray-400 hover:border-[var(--color-cyber-magenta)] hover:text-white cursor-pointer'}`}>
                      <div className={`font-mono font-black w-10 shrink-0 text-xl transition-colors ${isSelected ? 'text-[var(--color-acid-green)]' : isReadOnly ? 'text-gray-600' : 'group-hover:text-[var(--color-cyber-magenta)]'}`}>[ {letra} ]</div>
                      <span className="ml-2 font-bold uppercase tracking-wide">{optionText}</span>
                    </button>
                  );
                })}
              </div>
              {selectedOption !== null && !isReadOnly && (
                <div className="mt-8 animate-fade-in-up">
                  <p className="font-mono text-[var(--color-cyber-magenta)] font-bold text-sm tracking-widest mb-4 uppercase">&gt; JUSTIFIQUE SUA ESCOLHA_</p>
                  <textarea value={reasoning} onChange={(e) => setReasoning(e.target.value)} placeholder="DIGITE SEU RACIOCÍNIO AQUI (MIN. 10 CARACTERES)..." className="w-full h-32 bg-[#050505] border-[2px] border-[var(--color-cyber-magenta)] text-[var(--color-cyber-magenta)] p-4 font-mono text-sm uppercase placeholder:text-[#440044] focus:outline-none focus:border-white focus:text-white transition-colors resize-none" />
                  <button onClick={handleSubmit} disabled={reasoning.trim().length < 10 || isSubmitting} className="mt-6 w-full bg-[var(--color-cyber-magenta)] text-white font-display font-black text-2xl uppercase py-4 border-[3px] border-[var(--color-cyber-magenta)] hover:bg-black hover:text-[var(--color-cyber-magenta)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              {isReadOnly && (
                <div className="mt-8 border-[2px] border-[#333] bg-black p-4 text-center animate-fade-in-up">
                  <p className="font-mono text-gray-500 font-bold text-sm tracking-widest uppercase">[ MODO LEITURA: ESTE DESAFIO JÁ FOI ENCERRADO ]</p>
                </div>
              )}
            </div>
          </div>
      ...
      {isRankingOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-black border-[4px] border-yellow-400 shadow-[8px_8px_0px_0px_yellow-400] p-6 md:p-8 max-w-md w-full relative animate-fade-in-up">
            <button onClick={() => setIsRankingOpen(false)} className="absolute top-4 right-4 text-yellow-400 font-mono font-bold text-xl hover:text-white">[X]</button>
            <h2 className="font-display text-3xl font-black uppercase text-yellow-400 mb-6 tracking-tighter">LEADERBOARD_</h2>
            {loadingRanking ? (
                  ...
                  rankingData.map((student, index) => (
                    <div key={index} className={`flex items-center justify-between p-3 border-[2px] ${index === 0 ? 'border-yellow-400 bg-[#332200]' : 'border-[#333] bg-[#050505]'}`}>
                      <div className="flex items-center gap-4">
                        <span className={`font-display font-black text-xl ${index === 0 ? 'text-yellow-400' : 'text-gray-500'}`}>#{index + 1}</span>
                        <span className="font-mono font-bold text-white uppercase truncate max-w-[120px] md:max-w-[200px]">{student.name}</span>
                      </div>
